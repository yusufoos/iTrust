...
abstract public class BeanValidator<T> {
	 ...
	 * @return
	 */
	protected String checkDoubleGreater(String name, String value, double lower) {
		try {
			double doubleValue = Double.valueOf(value);
			if (lower <= doubleValue)
				return "";
		} catch (NumberFormatException e) {
			// just fall through to returning the error message
		}
		return name + " must be a decimal greater than or equal to " + lower;
	}
