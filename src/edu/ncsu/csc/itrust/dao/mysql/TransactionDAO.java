...
public class TransactionDAO {
	 ...
	 * @param loggedInRole TODO
	 * @param secondaryRole TODO
	 * @throws DBException
	 */
	public void logTransaction(TransactionType type, long loggedInMID, long secondaryMID, String addedInfo, String loggedInRole, String secondaryRole)
			throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			...
			ps = conn.prepareStatement("INSERT INTO transactionlog(loggedInMID, secondaryMID, "
					+ "transactionCode, addedInfo, loggedInRole, secondaryRole) VALUES(?,?,?,?,?,?)");
			ps.setLong(1, loggedInMID);
			ps.setLong(2, secondaryMID);
			ps.setInt(3, type.getCode());
			ps.setString(4, addedInfo);
			ps.setString(5,	loggedInRole);
			ps.setString(6, secondaryRole);
			ps.executeUpdate();
			ps.close();
		} catch (SQLException e) {
	...
	public List<TransactionBean> getTransactionsWithCriterias(String loggedInRole, String secondaryRole, int transactionCode, Timestamp startingDate, Timestamp endingDate) throws DBException {
		...
		String test = "transactionID >= 1";
		
		String loggedInRole2 = "";
		String secondaryRole2 = "";
		String startingDate2 = " AND timelogged >= \"" + startingDate + "\""; 
		String endingDate2 = " AND timelogged <= \"" + endingDate + "\""; 
		String transactionCode2 = "";
		
		if(!loggedInRole.equals("all"))
		{
			loggedInRole2 = " AND loggedInRole = \"" + loggedInRole + "\"";
			test += loggedInRole2;
		}
		if(!secondaryRole.equals("all"))
		{
			secondaryRole2 = " AND secondaryRole = \"" +secondaryRole + "\"";
			test += secondaryRole2;
		}
		if(transactionCode != 0)
		{
			transactionCode2 = " AND transactionCode = " + transactionCode;
			test += transactionCode2;
		}
		test += startingDate2;
		test += endingDate2;
		...
		ps = conn.
				prepareStatement("SELECT * FROM transactionlog WHERE " + test);
		
//		ps = conn.
//		prepareStatement("SELECT * FROM transactionlog WHERE transactionID >= 1 AND loggedInRole = ?"
//		+ "AND secondaryRole = ?"
//		+ "AND timelogged >= ?"
//		+ "AND timelogged <= ?"
//		+ "AND transactionCode = ?"
...
//		ps.setTimestamp(3, startingDate);
//		ps.setTimestamp(4, endingDate);
//		ps.setInt(5, transactionCode);
