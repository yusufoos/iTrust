...
<table border=1>
	...
	<%
		...
		for (TransactionBean t : list) {
			PersonnelBean pBean = pDAO.getPersonnel(t.getLoggedInMID());
			PatientBean p2Bean = p2DAO.getPatient(t.getLoggedInMID());
			String role = "tester";
 			if (pBean != null)
			{
					role = pBean.getRoleString();
			}
			if (p2Bean != null)
			{
				    role = "patient";
	...
	<tr>
		...
		<td><%= StringEscapeUtils.escapeHtml("" + (role)) %></td>
	</tr>
	<%
	}
	%>
